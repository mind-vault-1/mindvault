...
| ---------------------------- | ----------------------------------------------------------------------------------------------- | --------------------------------------------------------------- |
| `mindvault_setup_wallet`     | Create a Stellar wallet using the sponsored account protocol (optionally under a named profile) | `"Create a wallet for me"`                                      |
| `mindvault_wallet_info`      | Check the active profile, wallet address, USDC balance, and registration state                  | `"What's my wallet balance?"`                                   |
| `mindvault_use_profile`      | Switch the active wallet profile, creating it if new                                            | `"Switch to my buyer profile"`                                  |
| `mindvault_list_profiles`    | List all wallet profiles, marking the active one                                                | `"List my wallet profiles"`                                     |
...
| `mindvault_reset`            | Clear the active profile's credentials (or all profiles with `all=true`) from memory and disk   | `"Reset my agent credentials"`                                  |
...
To keep separate identities — e.g. testnet vs mainnet, or publisher vs buyer — in a single agent, use named wallet profiles. See **[docs/mcp-wallet-profiles.md](docs/mcp-wallet-profiles.md)**.
